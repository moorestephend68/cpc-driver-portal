...
# Hardcoded for your specific URL: https://cpc-driver.streamlit.app/
manifest_json = """
{
...
# Injects manifest into browser header
st.markdown(f"""
    <head>
...
st.markdown("""
    ...
    .btn-custom {
        ...
        color: white !important; /* Forced white for Android */
    }
...
try:
    ...
    if user_input:
        ...
        # KEY SEARCH: Find column by keywords
        id_col = next((c for c in roster.columns if any(k in c.lower() for k in ['employee', 'id', '#'])), roster.columns[0])
        roster['match_id'] = roster[id_col].apply(clean)
        match = roster[roster['match_id'] == u_id]

        if not match.empty:
            driver = match.iloc[0]
            rt_col = next((c for c in roster.columns if 'route' in c.lower()), roster.columns[0])
            ...
            # COMPLIANCE
            dot_col = next((c for c in roster.columns if 'dot' in c.lower()), None)
            cdl_col = next((c for c in roster.columns if any(k in c.lower() for k in ['dl', 'cdl'])), None)
            ...
            # PEOPLENET
            p_id = clean(driver.get('PeopleNet ID', 'N/A'))
            p_pw = str(driver.get('PeopleNet Password', 'N/A'))
            ...
            if not my_stops.empty:
                ...
                for _, stop in my_stops.iterrows():
                    ...
                    with st.expander(f"📍 Stop: {sid}", expanded=True):
                        ...
                        st.markdown(f'<a href="https://forms.office.com/Pages/ResponsePage.aspx?id=DQSIkWdsW0yxEjajBLZtrQAAAAAAAAAAAAO__Ti7fnBUQzNYTTY1TjY3Uk0xMEwwTE9SUEZIWTRPRC4u" class="btn-custom bg-red">🚨 Report Issue</a>', unsafe_allow_html=True)
