...
st.markdown("""
    ...
    /* Specific styling for Tomorrow's Note Box */
    .tom-dispatch-box {
...
def load_all_data():
    ...
    return (get_s(gids["roster"]), get_s(gids["dispatch"]), get_s(gids["schedule"]), 
            get_s(gids["links"]), get_s(gids["safety"]), get_s(gids["next_schedule"]), 
            get_s(gids["next_dispatch"]))
        ...
        if not match.empty:
            ...
            with tab_tom:
                ...
                for _, stop in t_stops.iterrows():
                    sid = clean_num(safe_get(stop, 'Store ID', 4))
                    addr = safe_get(stop, 'Store Address', 5)
                    with st.expander(f"📍 Store {sid.zfill(5)}"):
                        st.markdown(f"<div class='stop-detail-card'>{addr}</div>", unsafe_allow_html=True)
                        g_url = f"http://maps.apple.com/?q={addr.replace(' ','+')}"
                        st.markdown(f"<a href='{g_url}' class='btn-blue'>🌎 Navigation</a>", unsafe_allow_html=True)
