...
st.markdown("""
    ...
    .safety-box p { font-size: 19px !important; line-height: 1.4 !important; color: #1a1a1a !important; }
...
try:
    ...
    if user_input == "dispatch":
        ...
        if not match.empty:
            # --- MANDATORY SAFETY BRIEFING (iOS STABLE) ---
            safety_key = f"safety_read_{user_input}"
            if safety_key not in st.session_state: 
                st.session_state[safety_key] = False

            if not st.session_state[safety_key]:
                ...
                if st.button("✅ I HAVE READ AND UNDERSTAND", key="safety_btn_ios", use_container_width=True):
                ...
                if not st.session_state[f"route_confirmed_{user_input}"]:
                    ...
                    if st.button("Internal Confirmation (Bypass Form)", key="bypass_ios"): 
                ...
                st.markdown(f"<div class='peoplenet-box'><div style='font-size:18px; margin-bottom:10px; opacity:0.8;'>ELD Login</div><div style='display:flex; justify-content:space-around;'><div>ORG<br><span class='peoplenet-val'>3299</span></div><div>ID<br><span class='peoplenet-val'>{p_id}</span></div><div>PW<br><span class='peoplenet-val'>{p_id}</span></div></div></div>", unsafe_allow_html=True)
                ...
                elif not route_num: st.markdown(f"<div style='background-color:#e3f2fd !important; color: #0d47a1 !important; padding:20px; border-radius:10px; font-size:20px; font-weight:bold;'>📍 Assignment: {safe_get(driver, 'Route', 1)}</div>", unsafe_allow_html=True)
                else:
                    ...
                    if my_stops.empty: st.warning("⚠️ Refer to Dispatch Email")
                        ...
                        for _, stop in my_stops.iterrows():
                            ...
                            with st.expander(f"📍 Store {sid_5 if raw_sid != '0' else 'Relay'} (Arr: {arr})", expanded=True):
                                ...
                                <td><a href='https://www.google.com/maps/search/?api=1&query={addr.replace(' ','+')}' class='btn-blue'>🌎 Google</a></td></tr>
                                <tr><td><a href='truckmap://navigate?q={addr.replace(' ','+')}' class='btn-blue'>🚛 TruckMap</a></td>
                                <td><a href='https://wg.cpcfact.com/store-{sid_5}/' class='btn-blue'>🗺️ Map</a></td></tr>
                                </table><a href='https://forms.office.com/Pages/ResponsePage.aspx?id=DQSIkWdsW0yxEjajBLZtrQAAAAAAAAAAAAO__Ti7fnBUQzNYTTY1TjY3Uk0xMEwwTE9SUEZIWTRPRC4u' class='btn-red'>🚨 Issue</a>
                                """, unsafe_allow_html=True)
